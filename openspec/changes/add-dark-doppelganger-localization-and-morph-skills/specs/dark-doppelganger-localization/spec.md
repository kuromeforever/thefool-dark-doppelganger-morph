...
附属 SHALL 为精确来源 Jar 的全部 20 个现有语言键、4 个派生实体显示键和4个声音字幕提供非空简体中文，并 SHALL 为经字节码确认的 Boss、阶段、仆从、命令和废弃卷轴 literal 提供 translatable 映射。
...
发布 Jar SHALL 仅在来源命名空间包含 `lang/zh_cn.json` 与 `sounds.json`，并 SHALL 保持附属 `en_us` 与 `zh_cn` 键集合一致、值非空且描述真实技能行为。

#### Scenario: Thin Jar 资源检查
- **WHEN** 验证最终 reobf Jar
- **THEN** 未批准的来源模型、材质、音频、数据和语言文件均不存在，两个附属语言文件键集合完全一致
